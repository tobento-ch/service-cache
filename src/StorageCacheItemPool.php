...
/**
 * StorageCacheItemPool
 */
final class StorageCacheItemPool implements CacheItemPoolInterface, CanDeleteExpiredItems
{
    /**
     * @var array
     */
    ...
    /**
     * Create a new StorageCacheItemPool.
     *
    ...
    public function __construct(
        ...
        private string $expirationCol = 'expiration',
        private string $namespaceCol = 'namespace',
    ) {
        if (empty($namespace)) {
    ...
    public function getItem(string $key): CacheItemInterface
    {
        ...
        $item = $this->storage->table($this->table)->find($this->toStorageId($key));
        
        if (is_null($item)) {
            return $this->freshCacheItem(key: $key);
        }

        $cacheItem = unserialize($item[$this->dataCol] ?? '');
        
        $cacheItem->setClock($this->clock);
    ...
    public function clear(): bool
    {
        ...
        try {
            $this->storage->table($this->table)
                ->where($this->namespaceCol, '=', $this->namespace)
                ->delete();
    ...
    public function deleteItem(string $key): bool
    {
        ...
        try {
            $this->storage->table($this->table)
                ->where($this->idCol, '=', $this->toStorageId($key))
                ->delete();
        } catch (QueryException $e) {
            return false;
        }
    ...
    public function save(CacheItemInterface $item): bool
    {
        ...
        $item->setHit(true);
        $id = $this->toStorageId($item->getKey());
        
        try {
            $items = $this->storage->table($this->table)->updateOrInsert(
                [$this->idCol => $id], // where clauses
                [
                    $this->idCol => $id,
                    $this->dataCol => serialize($item),
                    $this->expirationCol => $item->getExpiration()?->getTimestamp(),
                    $this->namespaceCol => $this->namespace,
                ],
            );
    ...
    public function deleteExpiredItems(): bool
    {
        try {
            $this->storage->table($this->table)
                ->where($this->namespaceCol, '=', $this->namespace)
                ->where($this->expirationCol, '<', $this->clock->now()->getTimestamp())
                ->delete();
        } catch (QueryException $e) {
            return false;
        }
    ...
    /**
     * Returns the storage id for the specified cache key.
     *
     * @param string $key
     * @return string
     */
    private function toStorageId(string $key): string
    {
