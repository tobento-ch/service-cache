 ...
 * Psr16CacheItemPool
 */
final class Psr16CacheItemPool implements CacheItemPoolInterface, CanDeleteExpiredItems
{
    /**
     * @var array
     */
    ...
    /**
     * Create a new Psr16CacheItemPool.
     *
    ...
    {
        try {
            $cacheItem = $this->cache->get($this->toCacheKey($key));
        } catch (SimpleInvalidArgumentException $e) {
        ...
        try {
            $this->cache->delete($cacheItem->getKey());
        } catch (SimpleInvalidArgumentException $e) {
            throw new InvalidArgumentException($e->getMessage(), $e->getCode(), $e);
        }
    ...
    public function save(CacheItemInterface $item): bool
    {
        ...
        try {
            return $this->cache->set(
                key: $this->toCacheKey($item->getKey()),
                value: $item,
                ttl: $this->expirationToTtl($item->getExpiration()),
            );
        } catch (SimpleInvalidArgumentException $e) {
            throw new InvalidArgumentException($e->getMessage(), $e->getCode(), $e);
        }
    ...
    {
        if ($this->cache instanceof CanDeleteExpiredItems) {
            return $this->cache->deleteExpiredItems();
        }
     ...
     * @return null|int
     */
    private function expirationToTtl(null|DateTimeInterface $expiration): null|int
    {
        if (is_null($expiration)) {
            return null;
        }
        
        $interval = $this->clock->now()->diff($expiration);
        
        $seconds = DateTimeImmutable::createFromFormat('U', '0')->add($interval)->format('U');
